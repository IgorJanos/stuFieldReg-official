...
class Camera:
    ...
    def from_calib360_sample(
        ...
        self.position = np.array([sample.x, sample.y, sample.z], dtype='float')
        
        pan = sample.pan * np.pi / 180.
        tilt = sample.tilt * np.pi / 180.
        roll = sample.roll * np.pi / 180.
        self.rotation = np.array([
            [-np.sin(pan) * np.sin(roll) * np.cos(tilt) + np.cos(pan) * np.cos(roll),
             np.sin(pan) * np.cos(roll) + np.sin(roll) * np.cos(pan) * np.cos(tilt), np.sin(roll) * np.sin(tilt)],
            [-np.sin(pan) * np.cos(roll) * np.cos(tilt) - np.sin(roll) * np.cos(pan),
        ...
        # K 
        #f = self.focal_length_from_fov(fov_rad=sample.fov_v * np.pi / 180.0, d=height)
        f = height / (2.0 * np.tan(sample.fov_v * np.pi / (180.0 * 2.0)))
        self.xfocal_length = f
        self.yfocal_length = f
        self.calibration = np.array([
            [self.xfocal_length, 0, self.principal_point[0]],
            [0, self.yfocal_length, self.principal_point[1]],
            [0, 0, 1]
    ...
    @staticmethod
    def focal_length_from_fov(fov_rad, d):
        return 0.5 * d * (1.0 / np.tan(0.5 * fov_rad))

    

    def get_homography(self):
        It = np.eye(4)[:-1]
        It[:, -1] = -self.position  # (3, 4)
        P = self.calibration @ self.rotation @ It  #  # (3, 4)
        H = P[:, [0, 1, 3]]  # (3, 3)
        H = np.linalg.inv(H)
        return H        
    ...
    def reproject_point(self, point2D):
        # Normalized coordinates
        pn = np.array([
            (point2D[0]-self.principal_point[0])/self.xfocal_length, 
            (point2D[1]-self.principal_point[1])/self.yfocal_length, 
            1.0
        ], dtype=float)
        # Rotated point
        r = np.transpose(self.rotation) @ np.transpose(pn)
        # Rescale 
        t = -self.position[2]/r[2]
        r = r*t
    ...
    def draw_colorful_pitch(self, image, palette, thickness=1):
        """
        ...
        for key, line in polylines.items():
            ...
            for point in line[1:]:
                ...
                if 0 < projected[0] < self.image_width and 0 < projected[1] < self.image_height:
                    # BGR color
                    cv.line(image, (int(prev_point[0]), int(prev_point[1])), (int(projected[0]), int(projected[1])),
                            palette[key][::-1], thickness=thickness
                            )
                prev_point = projected
        return image
