...
from methods.kpsfr.models.modules import *
...
class KpSFR(nn.Module):
    ...
    def inference(self, images, lookup):        

        # Resample to quarter resolution
        b = images.shape[0]
        nh = images.shape[-2] // 4
        nw = images.shape[-1] // 4
        k = 91
        ...
        # Output
        out_logits = torch.zeros(size=((k+1),b,1,nh,nw), device=images.device)

        # Encode keys
        kf32, kf16, kf8, kf4 = self.encode_key(images[:,0:1])        
        #lookup = torch.arange(1, (k+1), step=1).to(images.device)
                
        # Get the predicted heatmaps
        for i_image in range(b):        
            out_mask_origin = self.segment_with_query(k, 
                kf32[i_image:i_image+1,0], 
                kf16[i_image:i_image+1,0], 
                kf8[i_image:i_image+1,0], 
                kf4[i_image:i_image+1,0], 
                lookup
            )        
        
            # Soft aggregation
            logits, prob = self.aggregate_inference(out_mask_origin, keep_bg=True)        
            out_logits[:,i_image] = F.interpolate(
                    logits, 
                    (nh, nw), 
                    mode='bilinear', 
                    align_corners=False
    ...
    def aggregate_inference(self, prob, keep_bg=False):
        # values of prob is from (0, 1)
        k = prob.shape
        new_prob = torch.cat([
