...
class RobustPreviewLogger(HomographyPreviewerLogger):
    def __init__(
        self,
        experiment,
        num_images: int = 4,
        ...
        if (homography_units == "yards"):
            ...
            self.scale = np.array([
                [ self.pf.length / playfield_shape[1], 0.0, 0.0],
                [ 0, self.pf.width / playfield_shape[0], 0.0],
                [ 0, 0, 1]
            ])
    ...
    def process_images(self, model, images):
        ...
        for i in range(images.shape[0]):
            ...
            image = self.to_image(images[i].detach().cpu())
